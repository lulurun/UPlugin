...
NPError OSCALL NP_GetEntryPoints(NPPluginFuncs* pFuncs)
{
	...
	TRACE_LOG("NP_GetEntryPoints enter");
	if(pFuncs == NULL)
	...
	TRACE_LOG("NP_GetEntryPoints leave");
...
{
	TRACE_LOG("NP_Initialize enter");
	if(pFuncs == NULL) {
		ERROR_LOG("pFuncs is NULL");
		return NPERR_INVALID_FUNCTABLE_ERROR;
	}

	if(HIBYTE(pFuncs->version) > NP_VERSION_MAJOR) {
		ERROR_LOG("version incompatible: " << pFuncs->version << " " << NP_VERSION_MAJOR);
		return NPERR_INCOMPATIBLE_VERSION_ERROR;
	}

	if(pFuncs->size < sizeof(NPNetscapeFuncs)) {
		ERROR_LOG("invalid functable: " << pFuncs->size << " " << sizeof(NPNetscapeFuncs));
		return NPERR_INVALID_FUNCTABLE_ERROR;
	}
...
{
	TRACE_LOG("NP_Shutdown enter");
	TRACE_LOG("NP_Shutdown leave");
	return NPERR_NO_ERROR;
}
